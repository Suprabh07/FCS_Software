...
2. Navigate to the \public\ folder inside the \msce-rocketry-app\ directory.
3. Extract the downloaded zip directly into public.
4. Ensure the folder structure is perfectly aligned like this:
   \msce-rocketry-app/public/map_tiles/{z}/{x}/{y}.png\
...
1. In the \msce-rocketry-app\ folder, create a new file named \.env\.
...
- **CSV Logs**: As soon as connection is established, all raw serial data is automatically logged into \msce-rocketry-app/public/flight_log.csv\ with exact PC timestamps.
...
1. **Motor Ignited**: Vertical Acceleration (\\) spikes > 15 m/s².
2. **Motor Burnout**: Sustained ignition drops to coast phase (\\ < 5 m/s²).
3. **Apogee Reached**: Vertical Velocity (\z\) drops below zero.
4. **Recovery Triggered**: Post-apogee shock spike detected (\\ > 10 m/s²).
